...
abstract class SearchableController extends Controller
{
    ...
    {
        $query
            ->where('subject_id', 'LIKE', "%{$word}%")
            ->orWhere('subject_name', 'LIKE', "%{$word}%");
    }
    ...
    function find(string $subject_id): Model
    {
        return $this->getQuery()->where('subject_id', $subject_id)->firstOrFail();
    }
}