...
@section('content')
@php
    session()->put('bookmarks.admin.add-approve', request()->fullUrl());
@endphp
<div class="container">
    <div class="mb-3">
        <a href="{{ url()->previous() }}" class="btn btn-secondary">กลับ</a>
    </div>
    <h2>Pending Subject Registration Approvals</h2>
            ...
            <tbody>
                @foreach($pen as $pending)
                    <tr>
                        ...
                        <td>
                            <form action="{{ route('admin.addapprove', $pending->id) }}" method="POST" class="d-inline">
                                @csrf
                                <input type="hidden" name="sub" value="{{ $pending->id }}">
                                <button type="submit" class="btn btn-success btn-sm">Approve</button>
                            </form>
                            
                            <form action="{{ route('admin.rejectadd', $pending->id) }}" method="POST" class="d-inline ml-2">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="sub" value="{{ $pending->id }}">
